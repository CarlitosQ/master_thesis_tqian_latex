...
Cavity modes are obtianed from \autoref{eq:RTG_RTP} for integer values of $\phi/2\pi$; for the lasing mode $m$ at threshold we have
\begin{equation}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=.6\linewidth]{figures/detuned_loading_principle.png}
...
The large enhancement of the modulation bandwidth, has been already observed to be closely related to a special behavior of the cavity Round Trip Phase (RTP) function [10], which is the phase factor $\varphi$ in \autoref{eq:RTG_RTP}. In order to illustrate the operation principle of the PPR, we take the simplified DBR laser structure as in \cite{montrosset2014laser}, which contains only a gain section and a grating. The parameter used for calculation is provided \autoref{tab:PP_resonance_operation_principle}. The python code used for calculation is provided in \autoref{sec:PP_resonance_cal}.

\begin{table}[ht]
    \centering
    \caption{Simplified DBR laser cavity parameters.}
